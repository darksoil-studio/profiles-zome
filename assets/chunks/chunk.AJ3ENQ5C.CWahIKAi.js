...
`,L="";function I(e){L=e}function ve(e=""){if(!L){const t=[...document.getElementsByTagName("script")],r=t.find(s=>s.hasAttribute("data-shoelace"));if(r)I(r.getAttribute("data-shoelace"));else{const s=t.find(a=>/shoelace(\.min)?\.js($|\?)/.test(a.src)||/shoelace-autoloader(\.min)?\.js($|\?)/.test(a.src));let i="";s&&(i=s.getAttribute("src")),I(i.split("/").slice(0,-1).join("/"))}}return L.replace(/\/$/,"")+(e?`/${e.replace(/^\//,"")}`:"")}var ge={name:"default",resolver:e=>ve(`assets/icons/${e}.svg`)},fe=ge,V={caret:`
  ...
  `},pe={name:"system",resolver:e=>e in V?`data:image/svg+xml,${encodeURIComponent(V[e])}`:""},me=pe,we=[fe,me],A=[];function be(e){A.push(e)}function ye(e){A=A.filter(t=>t!==e)}function k(e){return we.find(t=>t.name===e)}var _e=E`
  :host {
    display: inline-block;
    width: 1em;
    height: 1em;
